...
export default function StickyCartDrawer() {
  ...
  const content = (
    <>
      <button
        type="button"
        className="fixed bottom-60 right-6 z-9999 flex h-10 w-10 items-center justify-center rounded-full border border-border bg-card shadow-2xl transition hover:bg-primary hover:text-white sm:h-14 sm:w-14 sm:p-3"
      ...
      <div className={`fixed inset-0 z-9998 ${open ? "pointer-events-auto" : "pointer-events-none"}`}>
        ...
        >
          ...
          <div className="sticky bottom-0 border-t border-border bg-background px-4 py-4">
            ...
            <Link to="/cart" className="mt-3 block text-center text-sm text-primary underline" onClick={() => setOpen(false)}>
              View full cart
            </Link>
          </div>
        </div>
